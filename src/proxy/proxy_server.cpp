...
void ProxyServer::handle_connect_mitm(int cfd, int sfd, const std::string& host, const std::string& client_addr, const std::string& user) {
  ...
  while (true) {
    ...
    // Only top-level navigation requests are redirectable to avoid breaking subresources.
    // Script, image, and CORS requests should be rejected instead of redirected.
