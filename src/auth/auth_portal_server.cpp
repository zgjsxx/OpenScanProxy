...
std::string login_page_html(const std::string& return_to, const std::string& error, const std::string& current_user = "") {
  ...
  os << "<!doctype html><html><head><meta charset=\"utf-8\"><title>OpenScanProxy Login</title><style>"
     ...
     << ".pw-wrap input{width:100%;box-sizing:border-box;padding:12px 14px;padding-right:38px;margin:0;"
     << "border-radius:12px;border:1px solid #2f4f75;background:#0c1828;color:#eef4ff;outline:none;}"
     << ".pw-toggle{position:absolute;right:6px;top:50%;transform:translateY(-50%);"
     << "width:32px;height:32px;margin:0;padding:0;border:none;background:none;color:#8ea3bf;"
     << "cursor:pointer;font-size:18px;line-height:32px;text-align:center;"
     << "border-radius:8px;transition:background .2s,color .2s;}"
     << ".pw-toggle:hover{background:#1a3050;color:#eef4ff;}"
     << "</style></head><body><div class=\"card\"><h1>OpenScanProxy 认证</h1>"
      ...
      if (path.rfind("/login", 0) == 0 && method == "GET") {
        ...
        core::app_logger().log(core::LogLevel::Info,
                               "auth portal diagnostic: logout client_addr=" + client_addr +
                                   " client_ip=" + client_ip +
                                   " user=" + (current_user.empty() ? "-" : current_user));
