...
`PolicyEngine::evaluate_access()` (`src/policy/policy.cpp:169`) evaluates in this priority:

1. **Global whitelists** (any match → Allow, short-circuit):
   - user_whitelist → domain_whitelist → url_whitelist → url_category_whitelist
2. **Global blacklists** (any match → Block, short-circuit):
   - user_blacklist → domain_blacklist → url_blacklist → url_category_blacklist
3. **Per-user access rules** (iterated in order, first match wins):
   - User must match rule's users list (AND gate). If users list is empty, matches all.
   - Then conditions are OR: any one of domain / URL / category match triggers the rule's action (allow or block).
   - `src/policy/policy.cpp:55-84` (`evaluate_named_rule`)
4. **Default rule** — if nothing above matched, `default_access_action` is used

### MITM decision logic

`handle_connect_tunnel()` (`src/proxy/proxy_server.cpp:810-817`):

1. If `enable_https_mitm` is true → always MITM
2. Else if portal auth enabled → MITM only for unauthenticated users (IP not in portal_client_auth cache)
3. Otherwise → relay tunnel (no decryption)

IP cache (`portal_client_auth`) is updated when cookie or token auth succeeds in `handle_connect_mitm` / `handle_http_forward`.
